...
send_emails = false; % we are in a simulation mode, only send Slack messages
...
msg = sprintf('%s\n%s\n', 'This is the expected weekend water table for later today.', ...
...
to = {'m.krumin@ucl.ac.uk'};

%%  'Weekend water',...
% Write email to file

% we don't need any of these if not sending the email
% we also don't want to change the modification date of the mail file (to
% maintain correct refractory period)
if send_emails 
